...
class TestLinkedSpecificationItem {
	...
	void testLinkWithStatus() {
		...
		assertThat(this.linkedItem.getLinksByStatus(LinkStatus.COVERS), containsInAnyOrder(this.linkedItem));
	}
	...
	void testIsDefect_TrueBecauseOfBadLink() {
		for (final LinkStatus status : LinkStatus.values()) {
			...
			final boolean expected = (status != LinkStatus.COVERS) && (status != LinkStatus.COVERED_SHALLOW);
	...
	void testCountOutgoingLinks() {
		linkToNewItemsWithStatus(LinkStatus.COVERS, LinkStatus.PREDATED, LinkStatus.OUTDATED, LinkStatus.UNWANTED, LinkStatus.ORPHANED, LinkStatus.AMBIGUOUS);

		assertAll(() -> assertItemOutgoingLinkCount(this.linkedItem, 6), () -> assertItemOutgoingBadLinkCount(this.linkedItem, 5));
	}
	...
	void testCountIncomingLinks() {
		linkToNewItemsWithStatus(LinkStatus.COVERED_SHALLOW, LinkStatus.COVERED_PREDATED, LinkStatus.COVERED_OUTDATED, LinkStatus.COVERED_UNWANTED);

		assertAll(() -> assertItemIncomingLinkCount(this.linkedItem, 4), () -> assertItemIncomingBadLinkCount(this.linkedItem, 3));
	}
