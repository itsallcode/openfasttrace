...
format identifies its native comment lines: Markdown only forwards HTML comment
lines (`<!-- ... -->`) and RST only forwards `..` comment lines. The current
markup state machine continues to process every line. This keeps coverage-tag
recognition comment-safe without duplicating the shared parsing behavior or
changing file-selection priorities.
...
- [ ] Add one Markdown and one RST acceptance scenario. Each scenario must
      require import of comment-embedded full and configured short tags as a
      documentation artifact at the tag-line location while ordinary markup
      specification items remain unchanged; tag-shaped non-comment text must
      not create coverage items.
- [ ] Add one runtime `dsn` item per scenario in `doc/spec/design.md`. Specify
      ...
      with the matching path configuration only for native comment lines;
      retain dedicated importer selection and existing state-machine processing.
...
- [ ] In `MarkdownImporter`, forward only HTML comment lines matching the
      native `<!-- ... -->` comment form; in `RestructuredTextImporter`, forward
      only `..` comment lines. Continue to run the existing markup state
      machine for all input lines.
...
- [ ] Add Markdown and RST factory/importer tests for matching and non-matching
      ...
      and unchanged full-tag import when no configuration matches.
