...
class TestInitializingServiceLoader {
	...
	void testNoServicesRegistered() {
		...
		final InitializingServiceLoader<InitializableServiceStub, Object> voidServiceLoader = InitializingServiceLoader.load(InitializableServiceStub.class,
				context);
		final List<InitializableServiceStub> services = StreamSupport.stream(voidServiceLoader.spliterator(), false).collect(toList());
	...
	void testImporterFactoriesRegistered() {
		...
		final List<ImporterFactory> services = getRegisteredServices(ImporterFactory.class, context);
	...
	void testExporterFactoriesRegistered() {
		...
		final List<ExporterFactory> services = getRegisteredServices(ExporterFactory.class, context);
	...
	private <T extends Initializable<C>, C> List<T> getRegisteredServices(final Class<T> type, final C context) {
		final InitializingServiceLoader<T, C> serviceLoader = InitializingServiceLoader.load(type, context);
